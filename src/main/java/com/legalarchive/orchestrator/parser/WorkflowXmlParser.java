            ...
            for (Element el : children(root, "steps", null)) {
                ...
                if ("step".equals(tag)) {
                    ...
                    if (s.exec != null && !java.util.Arrays.asList(
                            "auto", "powershell", "cmd", "jar", "sql", "ifscopy", "filecopy", "setvar", "validate", "csvreplace", "encoding", "anonymize", "mask", "split", "safecopy", "dequote", "csvsql", "xlsx2csv", "diff", "sqlreport", "elarxml", "elarcheck", "json2csv", "tiffcompress", "ftpsend", "objpack")
                            .contains(s.exec.toLowerCase())) {
                        throw new IllegalArgumentException("Step '" + s.id + "': exec must be auto, powershell, cmd, jar, sql, ifscopy, filecopy, setvar, validate, csvreplace, encoding, anonymize, mask, split, safecopy, dequote, csvsql, xlsx2csv, diff, sqlreport, elarxml, elarcheck, json2csv, tiffcompress, ftpsend or objpack");
                    }
                    String ik = s.exec == null ? null : s.exec.toLowerCase();
                    boolean internal = "sql".equals(ik) || "ifscopy".equals(ik) || "filecopy".equals(ik) || "setvar".equals(ik) || "validate".equals(ik) || "csvreplace".equals(ik) || "encoding".equals(ik) || "anonymize".equals(ik) || "mask".equals(ik) || "split".equals(ik) || "safecopy".equals(ik) || "dequote".equals(ik) || "csvsql".equals(ik) || "xlsx2csv".equals(ik) || "diff".equals(ik) || "sqlreport".equals(ik) || "elarxml".equals(ik) || "elarcheck".equals(ik) || "json2csv".equals(ik) || "tiffcompress".equals(ik) || "ftpsend".equals(ik) || "objpack".equals(ik);
