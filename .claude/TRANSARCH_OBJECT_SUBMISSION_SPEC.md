...
4. **Compression.** §3.5 lists gzip / bzip2 / xz under "**Do** only use following compressions", but the required file names end in `.tar`, and a gzip'd archive is `.tar.gz`. Whether compression is permitted, expected, or merely tolerated for a `.tar`-named submission is not stated.
